              ...
              const GF ct, mult_chk_t* mult_chk)
{
  ...
  GF_sqr(mult_chk->x_shares[2], mult_chk->z_shares[2]);
  GF_sqr(mult_chk->z_shares[2], mult_chk->z_shares[2]);
  GF_sqr(mult_chk->z_shares[2], mult_chk->z_shares[2]);
  GF_sqr(mult_chk->z_shares[2], mult_chk->z_shares[2]);
  GF_sqr(mult_chk->z_shares[2], mult_chk->z_shares[2]);
  GF_sqr(mult_chk->z_shares[2], mult_chk->z_shares[2]);
  GF_sqr(mult_chk->z_shares[2], mult_chk->z_shares[2]);
  GF_mul_add(mult_chk->x_shares[2], aim2_constants[2], mult_chk->z_shares[2]);
  GF_transposed_matmul_add(mult_chk->x_shares[2], matrix_A[2],
                           mult_chk->x_shares[L]);
  ...
  for (size_t rep = 0; rep < TAU; rep++)
  {
    ...
    for (size_t party = 0; party < N; party += 4)
    {
      ...
      for (size_t j = 0; j < 4; j++)
      {
        ...
        GF_copy(tapes[j].t_shares[2], mult_chk[rep][party + j].x_shares[2]);
  ...
  for (size_t rep = 0; rep < TAU; rep++)
  {
    ...
    for (size_t party = 0; party < N; party++)
    {
      ...
      GF_mul_add(mult_chk[rep][party].z_shares[2], epsilons[2],
                 alpha_v_shares[rep][party][1]);
      GF_mul_add(mult_chk[rep][party].x_shares[3], epsilons[3],
                 alpha_v_shares[rep][party][0]);
      GF_mul_add(mult_chk[rep][party].z_shares[3], epsilons[3],
                 alpha_v_shares[rep][party][1]);
      ...
      for (size_t j = 0; j < 4; j++)
      {
        ...
        if (party + j == N - 1)
        {
          ...
          GF_add(tapes[j].t_shares[1], (uint64_t*)sig->proofs[rep].delta_ts_bytes[1],
                 tapes[j].t_shares[1]);
          GF_add(tapes[j].t_shares[2], (uint64_t*)sig->proofs[rep].delta_ts_bytes[2],
                 tapes[j].t_shares[2]);
        ...
        GF_copy(tapes[j].t_shares[2], mult_chk.x_shares[2]);
        ...
        GF_mul_add(mult_chk.x_shares[3], epsilons[3], alpha_share);
        GF_mul_add(mult_chk.z_shares[3], epsilons[3], v_shares[party + j]);
