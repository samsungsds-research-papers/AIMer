                 ...
                 uint8_t h_1[AIMER_COMMIT_SIZE])
{
  ...
  alignas(16) GF sbox_outputs[AIM2_NUM_INPUT_SBOX];
  ...
  for (size_t rep = 0; rep < TAU; rep++)
  {
    ...
    alignas(16) tape_t tapes[4];
    alignas(16) tape_t delta;
                       ...
                       uint8_t h_2[AIMER_COMMIT_SIZE])
{
  ...
  for (size_t rep = 0; rep < TAU; rep++)
  {
    alignas(16) GF epsilons[L + 1];
    hash_squeeze(&ctx_e, (uint8_t*)epsilons, AIM2_NUM_BYTES_FIELD * (L + 1));

    alignas(16) GF alpha = {0,};
    alignas(16) GF mult_buf[2][2 * N] = {0,};

    for (size_t party = 0; party < N; party++)
    {
      GF_copy(alpha_v_shares[rep][0][party], mult_buf[0][party << 1]);
      GF_copy(alpha_v_shares[rep][1][party], mult_buf[1][party << 1]);
    }

    POLY_mul_add_N(mult_chk[rep].x_shares[0], epsilons[0], mult_buf[0]);
    POLY_mul_add_N(mult_chk[rep].z_shares[0], epsilons[0], mult_buf[1]);

    POLY_mul_add_N(mult_chk[rep].x_shares[1], epsilons[1], mult_buf[0]);
    POLY_mul_add_N(mult_chk[rep].z_shares[1], epsilons[1], mult_buf[1]);

    POLY_mul_add_N(mult_chk[rep].x_shares[2], epsilons[2], mult_buf[0]);
    POLY_mul_add_N(mult_chk[rep].z_shares[2], epsilons[2], mult_buf[1]);

    POLY_red_N(mult_buf[0], alpha_v_shares[rep][0]);
    POLY_red_N(mult_buf[1], alpha_v_shares[rep][1]);
                 ...
                 const uint8_t* message, const size_t message_len)
{
  ...
  for (size_t rep = 0; rep < TAU; rep++)
  {
    ...
    GF alpha_v = {0,};
    GF alpha_v_shares[2][N], alpha_v_shares_hi[2][N];
    ...
    for (size_t party = 0; party < N; party++)
    {
      GF_copy(alpha_v_shares[0][party], mult_buf[0][party << 1]);
      GF_copy(alpha_v_shares[1][party], mult_buf[1][party << 1]);
    }

    POLY_mul_add_N(mult_chk.x_shares[0], epsilons[0], mult_buf[0]);
    POLY_mul_add_N(mult_chk.z_shares[0], epsilons[0], mult_buf[1]);

    POLY_mul_add_N(mult_chk.x_shares[1], epsilons[1], mult_buf[0]);
    POLY_mul_add_N(mult_chk.z_shares[1], epsilons[1], mult_buf[1]);

    POLY_mul_add_N(mult_chk.x_shares[2], epsilons[2], mult_buf[0]);
    POLY_mul_add_N(mult_chk.z_shares[2], epsilons[2], mult_buf[1]);

    POLY_red_N(mult_buf[0], alpha_v_shares[0]);
    POLY_red_N(mult_buf[1], alpha_v_shares[1]);
